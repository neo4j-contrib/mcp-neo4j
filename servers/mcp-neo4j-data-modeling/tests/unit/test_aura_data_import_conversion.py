...
from mcp_neo4j_data_modeling.data_model import DataModel, Node, Property, Relationship, PropertySource


@pytest.fixture
def sample_aura_data_import_model():
    """Load the sample Aura Data Import model from the JSON file."""
    json_file = Path(__file__).parent.parent / "resources" / "neo4j_importer_model_2025-06-30.json"
...
class TestPropertyConversion:
    ...
    def test_to_aura_data_import_key_property(self):
        ...
        assert result["nullable"] == False  # Key properties are not nullable
    ...
    def test_to_aura_data_import_non_key_property(self):
        ...
        assert result["nullable"] == True  # Non-key properties are nullable
...
class TestNodeConversion:
    ...
    def test_from_aura_data_import_simple_node(self, sample_node_label, sample_node_mapping):
    ...
    def test_from_aura_data_import_complex_node(self, sample_aura_data_import_model):
        ...
        country_node_label = sample_aura_data_import_model["dataModel"]["graphSchemaRepresentation"]["graphSchema"]["nodeLabels"][0]
        country_node_mapping = sample_aura_data_import_model["dataModel"]["graphMappingRepresentation"]["nodeMappings"][0]
        ...
        assert len(node.properties) == 12  # 13 total properties - 1 key = 12 non-key properties
    ...
    def test_to_aura_data_import_simple_node(self):
        ...
        node_label, key_property = node.to_aura_data_import("nl:0")
        ...
        assert node_label["properties"][0]["nullable"] == False
        assert node_label["properties"][1]["token"] == "name"
        assert node_label["properties"][1]["nullable"] == True
        
        # Check key property mapping
        assert key_property["node"]["$ref"] == "#nl:0"
    ...
    def test_node_mapping_property_not_found_raises_error(self, sample_node_label):
        ...
        with pytest.raises(ValueError, match="Property p:3 not found in node mapping"):
            Node.from_aura_data_import(sample_node_label, "subregion", invalid_mapping, "local")
...
class TestRelationshipConversion:
    ...
    def test_from_aura_data_import_relationship_with_properties(self):
        ...
        rel_mapping = {
            ...
            "propertyMappings": [
                {
                    "property": {"$ref": "#p:rel_1"},
                    "fieldName": "weight"
                },
                {
                    "property": {"$ref": "#p:rel_2"},
                    "fieldName": "since"
                }
            ]
        }
    ...
    def test_to_aura_data_import_simple_relationship(self):
        ...
        rel_type, rel_obj = relationship.to_aura_data_import("rt:1", "r:1", "n:0", "n:1")
    ...
    def test_to_aura_data_import_relationship_with_properties(self):
        ...
        rel_type, rel_obj = relationship.to_aura_data_import("rt:2", "r:2", "n:0", "n:1")
        
        # Check relationship type has properties
        assert len(rel_type["properties"]) == 2
        assert rel_type["properties"][0]["token"] == "relationshipId"
        assert rel_type["properties"][0]["nullable"] == False  # Key property
        assert rel_type["properties"][1]["token"] == "strength"
        assert rel_type["properties"][1]["nullable"] == True  # Non-key property
...
class TestDataModelConversion:
    ...
    def test_from_aura_data_import_node_key_properties(self, sample_aura_data_import_model):
    ...
    def test_round_trip_conversion_with_relationships(self):
        ...
        rel = Relationship(
            type="CONNECTS",
            start_node_label="Node1",
            end_node_label="Node2"
        )
    ...
    def test_metadata_preservation_round_trip(self, sample_aura_data_import_model):
        ...
        assert aura_metadata["dataModel_version"] == sample_aura_data_import_model["dataModel"]["version"]
        assert aura_metadata["constraints"] == sample_aura_data_import_model["dataModel"]["graphSchemaRepresentation"]["graphSchema"]["constraints"]
        assert aura_metadata["indexes"] == sample_aura_data_import_model["dataModel"]["graphSchemaRepresentation"]["graphSchema"]["indexes"]
        assert aura_metadata["configurations"] == sample_aura_data_import_model["dataModel"]["configurations"]
        ...
        for i, node in enumerate(data_model.nodes):
            node_id = f"n:{i}"
            original_vis_node = next((v for v in original_vis_nodes if v["id"] == node_id), None)
            if original_vis_node:
                assert "visualization" in node.metadata
                assert node.metadata["visualization"]["position"] == original_vis_node["position"]
        ...
        assert converted_back["dataModel"]["version"] == sample_aura_data_import_model["dataModel"]["version"]
        assert converted_back["dataModel"]["configurations"] == sample_aura_data_import_model["dataModel"]["configurations"]
        ...
        for original_vis_node in original_vis_nodes:
            converted_vis_node = next((v for v in converted_vis_nodes if v["id"] == original_vis_node["id"]), None)
        ...
        original_constraints = sample_aura_data_import_model["dataModel"]["graphSchemaRepresentation"]["graphSchema"]["constraints"]
        converted_constraints = converted_back["dataModel"]["graphSchemaRepresentation"]["graphSchema"]["constraints"]
        assert converted_constraints == original_constraints
        
        original_indexes = sample_aura_data_import_model["dataModel"]["graphSchemaRepresentation"]["graphSchema"]["indexes"]
        converted_indexes = converted_back["dataModel"]["graphSchemaRepresentation"]["graphSchema"]["indexes"]
    ...
    def test_export_without_metadata_uses_defaults(self):
        ...
        assert aura_dict["dataModel"]["graphMappingRepresentation"]["dataSourceSchema"] == {"type": "local", "tableSchemas": []}
        ...
        graph_schema = aura_dict["dataModel"]["graphSchemaRepresentation"]["graphSchema"]
        assert len(graph_schema["constraints"]) == 1
        assert len(graph_schema["indexes"]) == 1
        assert graph_schema["constraints"][0]["name"] == "TestNode_constraint"
        assert graph_schema["indexes"][0]["name"] == "TestNode_index"

    def test_visualization_reconstruction_with_new_nodes(self, sample_aura_data_import_model):
        ...
        original_vis_nodes = sample_aura_data_import_model["visualisation"]["nodes"]
        for original_vis_node in original_vis_nodes:
            converted_vis_node = next((v for v in vis_nodes if v["id"] == original_vis_node["id"]), None)
            if converted_vis_node:
                assert converted_vis_node["position"] == original_vis_node["position"]
        
        # Check that new node got a default position
        new_node_id = f"n:{len(data_model.nodes) - 1}"  # Last node should be the new one
...
class TestEdgeCases:
    ...
    def test_node_with_no_properties_mapping(self, sample_node_label):
        ...
        with pytest.raises(ValueError):
            Node.from_aura_data_import(sample_node_label, "subregion", empty_mapping, "local")
