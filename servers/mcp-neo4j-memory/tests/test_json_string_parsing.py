...
from mcp_neo4j_memory.core.tools import execute_tool, execute_tool_http
...
class TestJSONStringParsing:
    ...
    async def test_http_version_with_json_string_input(self, mock_memory):
        """Test that HTTP version of tools also handles JSON string input."""
        arguments_with_json_string = {
            "entities": '[{"name":"Entity1","type":"Person","observations":["Age: 30"]}]'
        }
        
        result = await execute_tool_http(mock_memory, "create_entities", arguments_with_json_string)
        
        # Verify the result
        assert result["success"] is True
        assert "data" in result
        assert len(result["data"]) == 1
        assert result["data"][0]["name"] == "Entity1"
