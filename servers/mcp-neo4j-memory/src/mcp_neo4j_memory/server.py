    ...
    async def create_relations(self, relations: List[Relation]) -> List[Relation]:
        for relation in relations:
            ...
            self.neo4j_driver.execute_query(
                query, {"relations": [relation.model_dump() for relation in relations]}
            )
...
async def create_mcp_server(neo4j_uri, neo4j_user, neo4j_password):
    ...
    async def handle_list_tools() -> List[types.Tool]:
        return [
            ...
            types.Tool(
                name="entity_exists",
                description="Check if an entity exists in the knowledge graph by name",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "name": {
                            "type": "string",
                            "description": "The name of the entity to check",
                        },
                    },
                    "required": ["name"],
                },
    ...
    async def handle_call_tool(
        ...
        try:
            ...
            if name == "create_entities":
                entities = [
                    Entity(**entity) for entity in arguments.get("entities", [])
                ]
                result = await memory.create_entities(entities)
                return [
                    types.TextContent(
                        type="text",
                        text=json.dumps([e.model_dump() for e in result], indent=2),
                    )
                ]
                ...
                result = neo4j_driver.execute_query(query, {"name": entity_name})
                exists = result.records[0].get("exists") if result.records else False
                return [
                    types.TextContent(type="text", text=json.dumps({"exists": exists}))
                ]
            else:
